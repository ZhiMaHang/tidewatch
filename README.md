...
> ⚠️ Claude 目前不确定会不会导致封号，目前用下来只要账号和网路环境干净，是不会封的。
...
`build-app.sh` 默认 ad-hoc 签名,签名标识随每次构建变化,macOS 会把每次重构建都当成"新应用",反复弹「Tidewatch 想要访问你的钥匙串中的密钥」。跑一次:

```bash
./scripts/dev-cert.sh
```

它会在登录钥匙串创建一个稳定的自签名身份 **Tidewatch Dev**(幂等,已存在则直接复用),之后 `build-app.sh` 检测到该身份就自动改用它签名。换身份后仍有**一次性**弹框(都只弹一次,点「始终允许」后重构建不再弹):

- 脚本设置证书信任时,系统可能要求输一次登录密码;
- 首次用该身份构建时,可能弹「codesign 想访问密钥」;
- 换新身份后首次运行 App,每个钥匙串条目各弹一次授权。

该身份只用于本机 dev 构建;发行链路(`build-dmg.sh`)保持 ad-hoc 不变,「更新后需重新授权钥匙串」的口径见[安装](#安装)一节。
